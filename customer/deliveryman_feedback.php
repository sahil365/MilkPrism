...
if (isset($_POST['submit'])) {
    ...
    $exe = $con->query("INSERT INTO deliveryman_feedback(customer_id,deliveryman_id,rating,message) VALUES('$c_id','$shop_id','$rating','$message')");
    // if ($exe) {
                    ...
                    <form method="post">
                        ...
                        <div style="margin-top:10px;" class="row">
                            ...
                            <select style="margin-left:15px;border: 1px solid #ced4da;border-radius:5px;background-color: #F8F8F8;" name="shop_id" required>
                                <?php
                                    $result = $con -> query("SELECT * from deliveryman");
                                ...
                                <option value="<?php echo $row -> deliveryman_id ?>"><?php echo $fullname ?></option>
                                <?php } ?>
                            </select>    
                        </div>
                        <div class="row" style="margin-top:20px">
                            <div style="padding-top:10px;" class="col-sm-3"><label>Enter Rating :</label></div>
                            <div class="col-sm-8"><input type="number" min="1" max="5" placeholder="Enter value out of 5" id="name" name="rating" class="form-control input-sm" required maxlength="1" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" ></div>
                        </div>
