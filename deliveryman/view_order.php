...
$deliveryman = $con->query("select * from deliveryman where deliveryman_id='$id'");
$shop = $deliveryman->fetch_object();
$shop_id = $shop->shop_id;

$category = $con->query("select * from orders where shop_id='$shop_id' and isDelivered='0' order by order_id desc ");
							...
							?>
								...
								<?php
									...
									<?php if (!$row->isDelivered) { ?>
												<td colspan="5"><a class="btn btn-danger" href="order_detail.php?o_id=<?php echo $row->order_id ?>">Delivery</a></td>
											<?php } else { ?>
